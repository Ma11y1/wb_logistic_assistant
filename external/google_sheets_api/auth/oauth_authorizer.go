...
func (c *OAuthAuthorizer) handler(w http.ResponseWriter, r *http.Request) {
	if r.URL.Path != c.callbackPath {
		w.Write([]byte(c.notifications.InvalidPath))
		http.NotFound(w, r)
	...
	if state != c.state {
		w.Write([]byte(c.notifications.InvalidState))
	...
	if code == "" {
		w.Write([]byte(c.notifications.InvalidCode))
	...
	if err != nil {
		w.Write([]byte(c.notifications.FailedExchangeCode))
