...
func (c *Client) getSessionToken(ctx context.Context, s *session.Session) (string, error) {
	...
	if s.SessionTokenExpired() {
		token, err := c.auth.RefreshSessionToken(ctx, s.Login(), s.AccessTokenString())
		if err != nil {
			return "", fmt.Errorf("failed refresh session token: %w", err)
		}
...
func (c *Client) refreshSession(ctx context.Context, s *session.Session) error {
	if s == nil {
		return fmt.Errorf("session is nil")
	}
	token, err := c.auth.RefreshSessionToken(ctx, s.Login(), s.AccessTokenString())
...
// RefreshSessionToken TODO: It won't update, you need to use a different API method. Find which method.
func (c *Client) RefreshSessionToken(ctx context.Context, login, accessToken string) (*models.AuthSessionToken, *models.UserInfo, error) {
	return c.GetSessionToken(ctx, login, accessToken)
}
