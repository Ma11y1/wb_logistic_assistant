...
// todo СДЕЛАТЬ ТУТ ОБРАБОТКУ КАПЧИ
...
func (s *AuthService) RefreshSessionToken(ctx context.Context, login, accessToken string) (*models.AuthSessionToken, error) {
	return s.GetSessionToken(ctx, login, accessToken)
}
