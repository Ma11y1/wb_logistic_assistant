...
func (i *Initializer) GetToken() (string, error) {
	...
	if token == "" {
		return "", errors.New("Initializer.TelegramBot.GetToken()", "no token found")
	}
	return token, nil
}
...
func (i *Initializer) SetToken(token string) error {
	if token == "" {
		return errors.New("Initializer.TelegramBot.SetToken()", "token is empty")
	}
...
func (i *Initializer) UpdateStorage() error {
	...
	if err != nil {
		return errors.Wrap(err, "Initializer.TelegramBot.UpdateStorage()", "failed to update storage")
	}
	return nil
}
